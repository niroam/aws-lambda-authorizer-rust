...
rm -r ./code/target/aarch64-unknown-linux-gnu/release/*.zip
pushd .
cd ./code/
...
while read -r fname; do
	...
	currgoahead=1
    popd >/dev/null
done <<< "$(find $(pwd)/code/target/aarch64-unknown-linux-gnu/release/ -mindepth 1 -maxdepth 1 -type f -name lambda* ! -name "*.*" -print)"
